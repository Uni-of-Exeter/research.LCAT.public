...
def scrape(row, timeout=20):
    ...
    # We can't return r once it's closed, so return the raw bytes too if you need it later.
    # Keeping return signature similar-ish: (None, d) isn't great, so return (raw, d).
    return raw, d
