...
const ClimateReport = ({
    ...
    season,
    applyCoastalFilter,
    selectedPages = ["climate"],
}) => {
    const shouldIncludePage = (pageId) => selectedPages.includes(pageId);

    return (
        <Document>
            <IntroPDF regions={regions} selectedPages={selectedPages} />
            ...
            {shouldIncludePage("hazards") && selectedImpactHazard && (
                <Page size="A4" style={styles.page}>
                    <HazardsPDF applyCoastalFilter={applyCoastalFilter} />
                </Page>
            )}

            {(shouldIncludePage("health-impacts") || shouldIncludePage("community-impacts")) &&
                selectedImpactHazard && (
                    <Page size="A4" style={styles.page}>
                        <ImpactsPDF
            ...
            {shouldIncludePage("adaptations") && (
                <Page size="A4" style={styles.page}>
                    <AdaptationsPDF selectedAdaptationHazards={selectedAdaptationHazards} filterName={filterName} />
                </Page>
            )}
        </Document>
    );
